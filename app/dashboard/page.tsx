...
export default async function DashboardPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-10 md:px-8">
      <p className="text-xs uppercase tracking-[0.25em] text-hubbay-gold">Dashboard</p>
      <h1 className="mt-2 text-4xl font-black text-hubbay-text">Admin Order Dashboard</h1>
      <p className="mt-2 text-sm text-hubbay-secondary">
        Monitor incoming orders and update order stages in real time.
      </p>
      {!isDatabaseConfigured ? (
        <p className="mt-3 text-sm text-hubbay-gold">
          Connect `DATABASE_URL` to view live data.
        </p>
      ) : null}
      <div className="mt-8">
